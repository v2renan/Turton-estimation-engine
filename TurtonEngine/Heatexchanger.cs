...
namespace TurtonEngine
{
    public class Heatexchanger:Fatores
    {
        ...
        public double custo()
        {
            ...
            setFm("Heat exchanger", type, mat);
            double Fmat = getFm();
            setFp(pres, "Heat exchanger", type);
            L = fCusto("Heat exchanger", type);
            double Fp = getFp();
            custoheatex = Math.Pow(10, ((L[0] + L[1] * Math.Log10(area) + L[2] * Math.Pow(Math.Log10(area), 2)))) * (B[0] + B[1] * Fp*Fmat);
            return custoheatex;
        }

        public Heatexchanger(string type, double area, double pres, string material )
        {
            this.type = type;
            this.area = area;
            this.pres = pres;
